...
def train(batch_size, x, t, r, lr, lr_step, lr_rate, steps):
    ...
    dynamic_lr = tf.train.exponential_decay(lr, global_step, lr_step, lr_rate, staircase=True)

    train_step = tf.train.AdamOptimizer(learning_rate=dynamic_lr).minimize(model.loss, global_step=global_step)
    ...
    with tf.Session() as session:
        ...
        for step in range(1, steps + 1):
            ...
            if step % 1000 == 0:
                ...
                saver.save(session, "model/" + save_timestamp + "/model", global_step=current_step)

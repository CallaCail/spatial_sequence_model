...
class Model:
    ...
    def input_layer(self):
        with tf.name_scope('input-layer'):
            self.input_extra = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, 20], name='input-extra')
            self.input_inflow = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.x, self.x, self.t], name='input-inflow')
            self.input_outflow = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.x, self.x, self.t], name='input-outflow')
            self.target_y = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.x, self.x], name='target-y')
    ...
    def prediction_layer(self):
        with tf.name_scope('prediction-layer'):
            ...
            self.flatten_extract_out = tf.layers.flatten(self.extract_out, name='flatten_extract')
            self.final_out = self.merge_layer_2(self.flatten_extract_out)

    def loss_layer(self):
        with tf.name_scope('loss-layer'):
            target = tf.layers.flatten(self.target_y, name='flatten_target')
            self.loss = tf.sqrt(tf.reduce_mean(tf.square(self.final_out - target)), name='loss')
    ...
    def merge_layer(self, inputs):
        ...
        self.merge_out = tf.nn.sigmoid(tf.reduce_sum(tf.stack([inputs, extra_out], axis=1), axis=1))
        return self.merge_out

    def merge_layer_2(self, inputs):
        with tf.name_scope('merge-layer'):
            ...
            merge_out = tf.reduce_sum(tf.stack([inputs, extra_out], axis=1), axis=1)
    ...
    def inflow_attention_layer_2(self):
        with tf.name_scope('attention-layer'):
            inflow_extract_out = self.flow_extract(self.input_inflow, name='inflow')
            flatten_inflow = tf.layers.flatten(inflow_extract_out, name='flatten_inflow_extract')
            flatten_outflow = tf.layers.flatten(self.outflow_out, name='flatten_inflow_extract')
    ...
    def multiply_layer(self, inputs, name):
        multiply_w = tf.Variable(tf.truncated_normal([1024], stddev=1), name=name + '_multiply_w')
    ...
    def flow_extract_layer(self, inputs, name):
        with tf.name_scope('flow-extract-layer'):
        ...
        return tf.reduce_sum(tf.concat(extract_list, axis=3), axis=3, keepdims=True, name=name)
    ...
    def flow_extract(self, inputs, out_channel=1, name=None):
        return tf.layers.Conv2D(filters=out_channel, kernel_size=[1, 1], activation=tf.nn.relu6, name=name + '_extract')(inputs)

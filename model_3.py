...
class Model:
    ...
    def merge_layer(self, inputs):
        ...
        self.merge_out = tf.nn.tanh(tf.reduce_sum(tf.stack([inputs, extra_out], axis=1), axis=1))
    ...
    def flow_extract(self, inputs, out_channel=1, name=None):
        return tf.layers.Conv2D(filters=out_channel, kernel_size=[1, 1], activation=tf.nn.tanh, name=name + '_extract')(inputs)

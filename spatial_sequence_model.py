...
class Model:
    ...
    def input_layer(self):
        ...
        self.input_series = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.x, self.x, self.r], name='input-series')
    ...
    def loss_layer(self):
        ...
        self.loss = tf.sqrt(tf.reduce_mean(tf.square(self.prediction - target)), name='loss')
        return self.loss

    def series_layer(self):
        inflow_series = tf.concat([self.extract_out, self.input_series], axis=3)
        inflow_series = tf.transpose(inflow_series, [0, 3, 1, 2])
